...
inline void WriteBlockHeader(const BlockMetadata &b, uint8_t *&current_data_ptr) {
...
inline void WritePrefixArea(const FSSTCompressionResult &prefix_compression_result, const BlockMetadata &b,
...
inline void WriteSuffixArea(const FSSTCompressionResult &suffix_compression_result, const BlockMetadata &b,
...
inline uint8_t * WriteBlock(uint8_t *block_start,
    ...
    // A) WRITE THE HEADER
    WriteBlockHeader(b, current_data_ptr);

    // B) WRITE THE PREFIX AREA
    WritePrefixArea(prefix_compression_result, b, prefix_area_start_index, current_data_ptr);

    // C) WRITE SUFFIX AREA
    WriteSuffixArea(suffix_compression_result, b, suffix_area_start_index, current_data_ptr);
    ...
    std::cout << "But b.block_size is: " << b.block_size << " bytes\n";
